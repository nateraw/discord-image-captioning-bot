...
def get_device(device_id: int = 0) -> D:
...
class Pipeline:
    def __init__(self, prefix_length=10, device=None):
        if not Path('conceptual_weights.pt').exists():
            gdown.download('https://drive.google.com/uc?id=14pXWwB4Zm82rsDdvbGguLfx9F8aM7ovT')
        self.prefix_length = prefix_length
        ...
        self.model.load_state_dict(torch.load('conceptual_weights.pt', map_location=torch.device('cpu')))
